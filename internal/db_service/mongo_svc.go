...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("STOMATOLOGY_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("STOMATOLOGY_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("STOMATOLOGY_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("STOMATOLOGY_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("STOMATOLOGY_API_MONGODB_DATABASE", "id-stomatology-al")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("STOMATOLOGY_API_MONGODB_COLLECTION", "stomatology")
	}

	if svc.Timeout == 0 {
		seconds := enviro("STOMATOLOGY_API_MONGODB_TIMEOUT_SECONDS", "10")
...
func (this *mongoSvc[DocType]) CreateDocument(ctx context.Context, id string, document *DocType) error {
    ...
    default: // other errors - return them
        fmt.Println("DB error:", result.Err())
        return result.Err()
    }

    fmt.Println("DB before:", db)
    _, err = collection.InsertOne(ctx, document)
    fmt.Println("DB after:", err)
    return err
}
